...
int main(int argc, char *argv[])
{
    ...
    {   
        flag_find = 0;
        // cout << endl<< "ii: " << inputwords[ii] << endl;
        for(int jj = 0; jj < codebookwords.size(); jj++)
        {
            // cout << codebookwords[jj] << bool(char(inputwords[ii]) == codebookwords[jj]);
            // cout << bool(isalpha(char(inputwords[ii] - 32)))<<char(inputwords[ii] - 32) <<","<<bool(isalpha(char(inputwords[ii])))<<char(inputwords[ii])<<"; ";
            if((char(codebookwords[jj]-32) == char(inputwords[ii]) && isalpha(char(inputwords[ii] + 32)) && isalpha(inputwords[ii])) //输入是大写字母
                && flag_find == 0)
            {
                // cout<<"大写"<<endl;
                if(islower(codebookwords[jj]))
                {
                    outputwords.push_back(numberbase[jj]);
                    cout << numberbase[jj] ;
                    outFile << char(numberbase[jj]);
                }else
                {
                    outputwords.push_back(char(numberbase[jj]+32));
                    cout << char(numberbase[jj]+32);
                    outFile << char(numberbase[jj]+32);
                }
                flag_find = 1;
                break;
            }else if((codebookwords[jj] == char(inputwords[ii]) && isalpha(char(inputwords[ii] - 32)) && isalpha(inputwords[ii]))
                && flag_find == 0) 
            {
                // cout<<"小写"<<endl;
                if(islower(codebookwords[jj]))
